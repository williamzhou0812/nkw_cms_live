...
const NkwCatering = () => {
    ...
    const [nkwCatering, setNkwCatering] = useState({
        title: '',
        logo: '',
...
const [editorState, setEditorState] = useState(
    ...
    useEffect(() => {
        axios.get('/api/nkwcatering').then(res =>  {
            ...
            setNkwCatering(res.data[0]);
            setId(res.data[0]._id);

            const blockHTML_nkwcatering_editor  = convertFromHTML(`${res.data[0].editor_context}`);
            const new_editor_context_state  = ContentState.createFromBlockArray(blockHTML_nkwcatering_editor);
            setEditorState(
    ...
    const SaveUpdate = (e) => {
        e.preventDefault();
        axios.put(`/api/nkwcatering/${_id}`, nkwCatering).then(res => {
  ...
  const handleFullWidthChange = event => {
    ...
    return (
        <Fragment>
                ...
                <ActiveLastBreadcrumb  rootPath="Explore" childOne="Divisions" childTwo="NKW Catering"   path=""  />
        ...
        {editBtn && (
           <Fragment>
                ...
                <input type="text" 
                        ...
                        value={nkwCatering.header_background_img} 
                        onChange={(e) => setNkwCatering({...nkwCatering, header_background_img: e.target.value}) }
                /> 

   <button type="button" onClick={handleClickOpen}>Libary / Browse Libary</button>
    <button type="button" onClick={() => setNkwCatering({...nkwCatering, header_background_img: ''})}>remove</button>
                    
                    <div>
                            <h3>Change Title</h3>
                            <input type="text" 
                                placeholder="Change page title" 
                                value={nkwCatering.title} 
                                onChange={(e) => setNkwCatering({...nkwCatering, title: e.target.value})}
                            /> 
                    </div>
                ...
                <div>
                    ...
                    <input type="text" 
                        placeholder="NAME" 
                        value={nkwCatering.contact.name}
                        onChange={e => 
                                {setNkwCatering({...nkwCatering,
                                            contact: { 
                                                ...nkwCatering.contact,
                                                name: e.target.value
                                            }
                    ...
                    <label htmlFor="PhoneOne">Phone One:</label>
                        <input 
                            type="text" 
                            placeholder="Job" 
                            value={nkwCatering.contact.phone_a} 
                            onChange={e => 
                                {setNkwCatering({...nkwCatering,
                                            contact: { 
                                                ...nkwCatering.contact,
                        ...
                        <div>
                            <label htmlFor="PhoneOne">Phone Two:</label>
                            <input 
                                type="text" 
                                placeholder="Job" 
                                value={nkwCatering.contact.phone_b} 
                                onChange={e => 
                                    {setNkwCatering({...nkwCatering,
                                                contact: { 
                                                    ...nkwCatering.contact,
                        ...
                        <input 
                            type="text" 
                            placeholder="Job" 
                            value={nkwCatering.contact.email} 
                            onChange={e => 
                                {setNkwCatering({...nkwCatering,
                                            contact: { 
                                                ...nkwCatering.contact,
                    ...
                    <div style={{textAlign: "left", padding: "0 10px"}}>
                        ...
                        <Editor
                            ...
                            value={editorState}
                            onChange={(e) => setNkwCatering({...nkwCatering, editor_context : draftToHtml(convertToRaw(editorState.getCurrentContent())).toString()})}
